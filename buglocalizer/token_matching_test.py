        ...
        if sum(matched_count) == 0:
            ...
            for src in src_files.values():
                ...
                if not common_tokens:
                    matched_num = len(set(pos_tagged_sum_desc) & set(src.comments['stemmed']))
                    common_tokens = matched_num - len(set(src.comments['stemmed']))
                
                if not matched_num:
                    matched_num = len(set(pos_tagged_sum_desc) & set(src.attributes['stemmed']))
                    common_tokens = matched_num - len(set(src.attributes['stemmed']))
